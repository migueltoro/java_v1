...
public class Centro {
	...
	public static Centro of() {
        if (Centro.centro == null)
            Centro.centro = Centro.of("ficheros_centro/alumnos.txt","ficheros_centro/profesores.txt",
                    "ficheros_centro/asignaturas.txt","ficheros_centro/matriculas.txt","ficheros_centro/asignaciones.txt");
        return Centro.centro;
    }
	
	public static Centro of(String alumnos, String profesores, String asignaturas, String matriculas,
			String asignaciones) {
		
		Centro.centro =  new Centro(
				Alumnos.of(alumnos), 
				Profesores.of(profesores), 
				Asignaturas.of(asignaturas), 
				Matriculas.of(matriculas),Asignaciones.of(asignaciones));
		return Centro.centro;
	}
	...
	public Map<Grupo,Set<Profesor>> profesoresDeGrupo() {
    	return this.asignaciones().todas().stream()
    			.collect(Collectors.groupingBy(a->Grupo.of(a.ida(),a.idg()),
    ...
    public Set<Profesor> profesoresDeGrupo(Integer ida, Integer idg) {
    	return this.asignaciones().todas().stream()
    			.filter(a->a.ida().equals(ida))
    			.filter(a->a.idg().equals(idg))
    			.map(a->this.profesores().profesor(a.dni()))
    			.collect(Collectors.toSet());  			
    }
    
	public Set<Profesor> profesores(Alumno a) {
    	Set<Grupo> ga = this.matriculas().todas().stream()
    			.filter(m->m.dni().equals(a.dni()))
    			.map(m->Grupo.of(m.ida(),m.idg()))
    			.collect(Collectors.toSet());
    	return this.asignaciones().todas().stream()
    			.filter(ap->ga.contains(Grupo.of(ap.ida(),ap.idg())))
    			.map(ap->this.profesores().profesor(ap.dni()))
    			.collect(Collectors.toSet());  		
    }
	...
	public Map<Integer,Integer> numGruposPorAsignatura() {
    	return this.matriculas().todas().stream()
    			.map(m->Grupo.of(m.ida(),m.idg()))
    			.distinct()
	...
	public List<String> nombresAlumnosTop(Integer n){
		return this.alumnos().todos().stream()
			.filter(a-> a.fechaDeNacimiento().getYear() > 2000)			
	...
	public Set<Asignatura> obtenerAsignaturasComunes(List<Alumno> alumnos) {
		...
		Map<Alumno, Set<Asignatura>> as = this.matriculas().todas().stream()
				.filter(m -> dnis.contains(m.dni()))
    ...
    public static void main(String[] args) {
    	...
    	System.out.println(c.numProfesoresPorEdad().entrySet().stream()
    			.map(x->x.toString())
    			.collect(Collectors.joining(",","{","}")));
    	System.out.println("_____________________");
    	System.out.println(c.numGruposPorAsignatura().entrySet().stream()
    			.map(x->x.toString())
    			.collect(Collectors.joining(",","{","}")));
