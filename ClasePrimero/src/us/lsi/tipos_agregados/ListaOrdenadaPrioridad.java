...
public class ListaOrdenadaPrioridad<V, P> implements Heap<V, P> {
	
	private List<V> elements;
	private List<P> priorities;
	private Comparator<P> comparator;
	
	public static <V, P extends Comparable<P>> ListaOrdenadaPrioridad<V, P> of() {
		return new ListaOrdenadaPrioridad<V, P>(Comparator.naturalOrder());
	}
	
	public static <V, P> ListaOrdenadaPrioridad<V, P> of(Comparator<P> comparator) {
		return new ListaOrdenadaPrioridad<V, P>(comparator);
	}
	
	public ListaOrdenadaPrioridad(Comparator<P> comparator) {
		super();
...
//  Obtiene el índice del elemento que es menor o igual que e y tal que el siguiente elemento es mayor que e
	...
	@Override
	public V remove() {
		assert !this.isEmpty() : "The heap is empty";
